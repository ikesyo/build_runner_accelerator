...
### Shared package asset index for tracked globs

On 2026-09-23, the Rust workspace's cached package asset index was changed to
share its immutable `Vec<String>` through `Arc`. Before this change, each
tracked glob cloned every asset ID in the package before applying its literal
prefix. That repeated deep copy dominated the no-op glob stage on the scaled
JSON Serializable fixture.

The paired no-op measurements used Dart 3.13.4, Rust 1.98.1,
`build_runner` 2.16.1, `json_serializable` 6.14.1, and `source_gen` 4.2.4. The
fixture had 3,500 Dart inputs, 7,000 persisted actions, and 3,500 generated
outputs. Both binaries read the same graph and cache; runs were interleaved
after one clean build, with page cache warm and runtime metrics disabled for
wall-time measurements.

| Implementation | No-op wall time (median of 3) | Individual runs | Peak RSS (median) |
| --- | ---: | --- | ---: |
| Deep-cloned package index | 3,213 ms | 3,213 / 3,161 / 3,468 ms | 58,504 KiB |
| Shared `Arc` package index | 910 ms | 902 / 910 / 983 ms | 58,600 KiB |

The no-op wall median fell by 71.7%. With runtime metrics enabled for one
diagnostic run, Rust `initial_globs_us` fell from 2,508,287 to 103,729; these
instrumented wall times are not used in the table. No-op dispatches zero worker
actions. A stock clean run generated the same 3,500 output files with identical
bytes to the accelerator clean output (1,849,006 generated bytes).

This result covers one scaled JSON Serializable workspace on one machine. It is
not a release-wide guarantee. The clean runs were used to create the persisted
