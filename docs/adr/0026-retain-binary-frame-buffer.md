...
envelopeのraw部分を別の`Vec<u8>`へ切り出していた。FBRRのdecodeでは、そのraw
...
- `raw_bytes()`でraw部分をviewし、build result decodeは各outputへ必要な一回だけ
  `Vec<u8>`を作る。
- JSON frameのdecode経路と、FBRRのmetadata schema・output順序・strict capability境界は
  変更しない。
- outputをframe bufferへborrowさせるzero-copy API、shared memory、mmapは今回導入しない。

## Consequences

- FBRR受信時のraw payload全体の中間copyをなくし、output復元のcopy回数を二重から一回に
  減らせる。
...
- raw部分を従来どおり別`Vec`へcopyする: 実装は単純だが、FBRRのpayloadサイズに比例する
