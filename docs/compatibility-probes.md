...
2. Add explicit trigger metadata/evaluation only if invoking a non-triggered
   builder proves observably different from stock or causes a builder to fail
   when it receives an input that stock would skip. The current built_value
   fixture already matches stock output presence, so this remains outside the
